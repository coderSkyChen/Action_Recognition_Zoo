...
class BNInception_3c(nn.Module):
    def __init__(self, model_path='model_zoo/bninception/bn_inception.yaml', num_classes=101,
        ...
        super(BNInception_3c, self).__init__()
        ...
        for l in layers:
            ...
            if l['expr'] == 'inception_3c_double_3x3_1_bn<=ReLU<=inception_3c_double_3x3_1_bn':
                print('early break!')
                break
        new_s = {}
        own_s = self.state_dict()
        old_m = torch.utils.model_zoo.load_url(weight_url)
        for name, param in old_m.items():
            if name in own_s:
                new_s[name]=param
