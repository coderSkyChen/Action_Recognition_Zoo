...
I apply a TV-L1 optical flow algorithm, pixel values are truncated to the range \[-20, 20\], then rescaled between 0 and 255, each optical flow has two channels:horizontal and vertical component. Note that the fps in original dataset is 12, which is too fast for optical flow computing for something-something in practice, so i sample frame at 6fps.
